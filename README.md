...
`XPocket` 是PerfMa为终结性能问题而生的开源插件容器，它将定位或者解决各种性能问题的常见Linux命令，JDK工具，知名性能工具等适配成各种XPocket插件，并让它们可以相互联动一键解决特定的性能问题。
目前XPocket插件生态已经实现了覆盖CPU，系统进程/线程，内存，网络，IO，Web容器，数据库，磁盘，JVM等场景，包括hsdb,jdb,vjmap,Perf,top,greasy,ss,sar等在内的几十个个插件化集成。常用诊断场景所需工具包齐全啦～

当你遇到以下类似问题时，`XPocket`可以帮助到你：
1. 系统存在性能问题，不知道怎么办？
2. 问题排查时一头雾水，不知该何去何从？
3. 有了思路，但是不知道哪个工具好？
4. 好不容易找到一个工具，发现输入输出不合理，换一个又太麻烦？
5. 自己开发的小工具想要让更多人用？
6. 排查问题的思路想要帮助更多的人？
